...
<div class="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Time Slots</h2>
    <a href="{{ route('admin.time-slots.create') }}" class="bg-[#45A247] text-white px-4 sm:px-6 py-2 rounded-lg font-semibold hover:bg-[#3a8a3c] transition text-sm sm:text-base w-full sm:w-auto text-center">
    ...
    <div class="lg:hidden divide-y divide-gray-200">
        ...
        <div class="p-4">
            <div class="flex justify-between items-start mb-2">
                <div>
                    <h3 class="font-semibold text-gray-900">{{ $timeSlot->formatted_time }}</h3>
                    @if($timeSlot->label)
                        <p class="text-sm text-gray-500">{{ $timeSlot->label }}</p>
                    @endif
                </div>
            ...
            <div class="text-sm text-gray-500 mb-3">
                Sort Order: {{ $timeSlot->sort_order }}
            </div>
        ...
        @empty
        <div class="p-6 text-center text-gray-500">No time slots found</div>
        @endforelse
    </div>
