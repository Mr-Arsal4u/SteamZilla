...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
        <h3 class="text-lg font-semibold text-gray-900">Send Us a Query</h3>
        <p class="text-sm text-gray-600 mt-1">Have a question or need assistance? We're here to help!</p>
    </div>

    <form action="{{ route('user.contact.submit') }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
...
<div class="mt-6 bg-white rounded-lg shadow overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">Previous Queries</h3>
    </div>
    <div class="divide-y divide-gray-200">
        ...
        <div class="p-6 hover:bg-gray-50 transition">
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <p class="text-gray-900 mb-2">{{ Str::limit($query->message, 150) }}</p>
                ...
                <div>
                    @if($query->is_read)
                        <span class="px-3 py-1 bg-green-100 text-green-800 text-xs rounded-full">Read</span>
                    @else
                        <span class="px-3 py-1 bg-orange-100 text-orange-800 text-xs rounded-full">Pending</span>
                    @endif
                </div>
            </div>
        </div>
